...
hive -e " select min(price),max(price),avg(price) from ${table1} "  >>  "./OutputFile.txt"
hive -e " select property_type,count(property_type) from ${table1} group by property_type "  >>  "./OutputFile.txt"
hive -e " select id,property_type,room_type,price,case when price>avg(price) then 'High Price' else 'Low Price' end as lh from ${table1} group by id,property_type,room_type,price "  >>  "./OutputFile.txt"
hive -e " select r.location_id,l.city,l.state,l.pincode,count(r.location_id) from ${table1} r join ${table2} l on r.location_id=l.id group by r.location_id,l.city,l.state,l.pincode "  >>  "./OutputFile.txt"
hive -e " select r.location_id,l.city,min(r.price),max(r.price),avg(r.price) from ${table1} r join ${table2} l on r.location_id=l.id group by r.location_id,l.city"  >>  "./OutputFile.txt"